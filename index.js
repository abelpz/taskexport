...
import { sendTasksTree } from './res/checking.sendTw.js';
...
let sendTwTasks = async resPath => {
    
    if (path.extname(resPath) !== '.tsv') return
    fs.readFile(resPath, 'utf-8', async function (err, content) {

        if (err) {
            console.error(err);
            return
        }
        const tasksTree = createTasksTree(tsvParse({ tsv: content }))

        sendTasksTree(tasksTree, 1)
        //sendInterval(tasksTree, 9, 120000)
        //tasksTree.forEach((task, index) => {console.log("index: ", index, ", name: ", task.name)})
    })
}
sendTwTasks(base + repo + res)